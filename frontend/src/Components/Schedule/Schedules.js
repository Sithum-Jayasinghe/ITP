...
// Example coordinates for cities (replace with your real coordinates)
const cityCoords = {
...
const Schedules = () => {
  ...
  const getSchedules = () => {
    ...
    Axios.get("http://localhost:3001/api/schedules")
      .then((res) => { setSchedules(res.data?.response || []); setLoading(false); })
      .catch(() => { setSnackbar({ open: true, message: "Failed to fetch schedules", severity: "error" }); setLoading(false); });
  };
  ...
  const deleteSchedule = (data) => {
    Axios.post("http://localhost:3001/api/deleteschedule", data)
      .then(() => { getSchedules(); setSnackbar({ open: true, message: "Schedule deleted successfully", severity: "success" }); })
  ...
  const deleteBooking = (data) => {
    Axios.post("http://localhost:3001/api/deletebooking", data)
      .then(() => { getBookings(); setSnackbar({ open: true, message: "Booking deleted successfully", severity: "success" }); })
  ...
  const handleSearchChange = (event) => { debouncedSearch(event.target.value); setPage(1); };
  ...
  const filteredSchedules = useMemo(() => {
    ...
    return schedules.filter(
      (s) => s.flightName.toLowerCase().includes(lowerSearch)
  ...
  const generatePDF = () => {
    ...
    const tableRows = sortedSchedules.map((row) => [row.id,row.flightName,row.departure,row.arrival,row.dtime,row.atime,row.aircraft,row.seats,row.status]);
    autoTable(doc, { head: [tableColumn], body: tableRows, startY: 35, styles: { fontSize: 10 }, headStyles: { fillColor: [22, 160, 133], textColor: [255,255,255] }, alternateRowStyles: { fillColor: [240,248,255] }, theme: "striped" });
    doc.setFontSize(10);
  ...
  return (
    <Box sx={{ width: "calc(100% - 100px)", margin: "auto", marginTop: 10, bgcolor: darkMode ? "#121212" : "#f9fafb", color: darkMode ? "#e0e0e0" : "#222", minHeight: "100vh", pb: 5, borderRadius: 3, boxShadow: darkMode ? "0 4px 12px rgba(0,0,0,0.7)" : "0 4px 12px rgba(0,0,0,0.1)", transition: "background-color 0.3s ease, color 0.3s ease" }}>
      ...
      <Box sx={{ mb: 3, textAlign: "right" }}>
        <FormControlLabel control={<Switch checked={darkMode} onChange={() => setDarkMode(!darkMode)} />} label="Dark Mode" sx={{ color: darkMode ? "#e0e0e0" : "#444", fontWeight: "600" }} />
      </Box>

      {/* Schedule Form */}
      <SheduleForm addSchedule={addSchedule} updateSchedule={updateSchedule} submitted={submitted} data={selectedSchedule} isEdit={isEdit} darkMode={darkMode} />

      {/* Search & PDF */}
      <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mt: 5, mb: 3, flexWrap: "wrap", gap: 2 }}>
        <TextField label="🔍 Search Flights / Status / Destination" variant="outlined" size="medium" onChange={handleSearchChange} sx={{ flex: "1 1 60%", minWidth: 280 }} />
      ...
      {loading ? (
        ...
        <>
          {/* Schedule Table */}
          <ScheduleTable rows={paginatedSchedules} selectedSchedule={(data) => { setSelectedSchedule(data); setIsEdit(true); }} deleteSchedule={(data) => { setScheduleToDelete(data); setOpenDeleteDialog(true); }} onSort={handleSort} sortConfig={sortConfig} darkMode={darkMode} />

          {/* Pagination */}
          <Box sx={{ display: "flex", justifyContent: "center", mt: 4 }}>
            <Pagination count={Math.ceil(sortedSchedules.length / PAGE_SIZE)} page={page} onChange={(e, value) => setPage(value)} color="primary" shape="rounded" size="large" />
          </Box>

          {/* Map */}
          <Box sx={{ mt: 6, height: 400, borderRadius: 2, overflow: "hidden" }}>
            ...
            <MapContainer center={[20, 0]} zoom={2} style={{ width: "100%", height: "100%" }}>
              ...
              {sortedSchedules.map((s, index) => {
                const dep = cityCoords[s.departure];
                const arr = cityCoords[s.arrival];
                if (!dep || !arr) return null;
                return (
                  <React.Fragment key={index}>
                    <Marker position={dep}><Popup>{s.flightName} departs from {s.departure}</Popup></Marker>
                    <Marker position={arr}><Popup>{s.flightName} arrives at {s.arrival}</Popup></Marker>
                    <Polyline positions={[dep, arr]} color="blue" />
                  </React.Fragment>
                );
              })}
            </MapContainer>
          ...
          <Box sx={{ mt: 6 }}>
            ...
            <BooksTable rows={bookings} selectedBooking={(data) => console.log("Selected booking:", data)} deleteBooking={deleteBooking} darkMode={darkMode} />
          </Box>
        </>
      )}
      ...
      <Dialog open={openDeleteDialog} onClose={() => setOpenDeleteDialog(false)}>
        ...
        <DialogActions>
          ...
          <Button color="error" variant="contained" onClick={() => { deleteSchedule(scheduleToDelete); setOpenDeleteDialog(false); }}>Yes, Delete</Button>
        </DialogActions>
      </Dialog>
